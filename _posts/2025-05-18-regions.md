...
Let's start by visualizing our previous low-pass filtering results to build intuition.
...
After applying the same filter to all genes in the dataset, we can perform the same eigendecomposition shown above to obtain region components.
We find that the resulting components indeed show an interesting layered pattern.
However, there's an interesting pattern that didn't emerge in our simulated data.
Notice how the region components themselves appear like a [frequency basis](/blog/2025/graph-fourier) along an axis from the upper to lower edges of the tissue.
