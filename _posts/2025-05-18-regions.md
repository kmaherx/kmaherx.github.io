...
<!-- We can visualize $\mathbf{\bar C}$ as an image (\textbf{Figure \ref{fig:cov_lowpass}A}).
...
\begin{figure}[t]
\centering\includegraphics[width=\textwidth]{MIT-thesis-template/chapter3_figs/simulations/region_covariance.png} 
\caption{
\textbf{Visualization of low-pass gene-gene covariance and components.}
\textbf{A)} The low-pass covariance matrix $\mathbf{\bar C}$ visualized.
Gene markers are grouped by ground truth pattern (e.g. see the four red blocks along the top left diagonal that correspond to the four ground truth regions).
\textbf{B)} The covariance eigenbasis $\mathbf{\bar U}$ (i.e. loadings matrix in PCA) visualized.
The box inset corresponds to the zoom on the right, which emphasizes top components describing molecular relationships between regions.
The first component is dismissed as a scaling artifact.
\label{fig:cov_lowpass}
}
\end{figure}
...
% Furthermore, we wish to prioritize the most prominent patterns by assigning each cell a single value, i.e. projecting each cell into a one-dimensional space.
...
\begin{equation} \label{eq:quadform_cov_lows}
