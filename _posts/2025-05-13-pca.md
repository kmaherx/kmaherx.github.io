...
Viewed as a dimension reduction algorithm, the goal of PCA is to reduce "unnecessary" information, thereby emphasizing more prominent patterns in the data.
But how do we specify that mathematically?

First, we need to explicitly specify **what patterns we are interested in**.
...
Now consider that we dont care about the overall magnitude of $\mathbf{v}$.
...
(This is literally the same math introduced in a [previous post](/_posts/2025-05-18-graph-fourier.md) except that it relies on the adjacency matrix rather than the Laplacian matrix).
