...
toc:
  ...
  - name: Simulated data
    subsections:
      ...
      - name: Region gradients
      - name: Region clusters
...
While we will continue to refer to this as covariance, we will omit all mean centering for simplicity (since it doesn't appear to affect the results much, at least in my hands).
...
Ultimately, we seek to distill these blocks into a simpler representation -- perhaps by grouping genes within each block into a single "region program".
It turns out we can do this by eigendecomposing $\mathbf{C}$, i.e. performing PCA.
For a primer on PCA, see my [previous post](/_posts/2025-05-13-pca.md).
Let $\mathbf{u} \in \mathbb{R}^{n}$ represent this desired representation.
...
$$
\mathbf{S} = \mathbf{X} \mathbf{U} \in \mathbb{R}^{n \times 3}
$$
...
<figure style="text-align: center;">
  ...
  <figcaption><strong>Figure 3:</strong> Region gradient visualized in region component space.</figcaption>
</figure>
...
<figure style="text-align: center;">
  ...
  <figcaption><strong>Figure 3:</strong> Region gradient visualized in tissue space.</figcaption>
</figure>
...
<figure style="text-align: center;">
  ...
  <figcaption><strong>Figure 3:</strong> Region clusters visualized in region component space.</figcaption>
</figure>

<figure style="text-align: center;">
  <img src="/assets/figures/regions/region_clusters.png"
       alt=""
       style="width:50%; display: block; margin: 0 auto;">
  <figcaption><strong>Figure 3:</strong> Region clusters visualized in tissue space.</figcaption>
</figure>
