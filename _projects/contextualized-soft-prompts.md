...
Rather than going backward by projecting soft prompt embeddings onto the vocabulary, we can go forward by passing them through the model and asking what they mean. That is the behavioral criterion.
...
Under contextualization, the reconstruction error at layer 17 drops back to baseline. The soft prompt's representation now decomposes into the same features the model uses when reading the ground-truth instruction text directly. For the concise target, the contextualized soft prompt activates features like [8979](https://www.neuronpedia.org/gemma-3-4b-it/17-gemmascope-2-res-16k/8979) ("conciseness / summary"), [3296](https://www.neuronpedia.org/gemma-3-4b-it/17-gemmascope-2-res-16k/3296) ("length / brevity"), and [10440](https://www.neuronpedia.org/gemma-3-4b-it/17-gemmascope-2-res-16k/10440) ("the short answer is"). These are concept-level features that prepend does not engage. We see the same pattern for the Spanish target, where the contextualized soft prompt activates language-specific features like [9262](https://www.neuronpedia.org/gemma-3-4b-it/17-gemmascope-2-res-16k/9262) ("Spanish questions") and [146](https://www.neuronpedia.org/gemma-3-4b-it/17-gemmascope-2-res-16k/146) ("non-English response generation").

<!-- TODO_NUMBERS: fill in hard-prompt Jaccard counts from context_diffing data -->
For the concise target, [N_gt_concise] features activate for the ground-truth instruction. Prepend shares [N_prep_concise] of them; the contextualized soft prompt shares [N_ctx_concise]. The top shared features are the concept-level ones listed above, such as [8979], [3296], and [10440]. Contextualization appears to recover the concepts the instruction encodes, not just its surface form.
...
The prepended soft prompt's output on the perfect-morning prompt repeats: "a scrim of grey, a scrim of wet, a scrim of bone." This is a known artifact of directly steering activations off the natural language manifold. The contextualized version does not repeat.
...
Soft prompts are powerful and lightweight, but opacity has made them a safety concern. Contextualization during training appears to dissolve both halves of this tension. The behavioral criterion holds: the model describes contextualized soft prompts accurately across both hard-prompt and steering-vector targets, reaching for "Be concise," "Speak in Spanish," and "Emulate J.G. Ballard" when appropriate. The mechanistic criterion holds: contextualized soft prompts' representations decompose into the same concept-level features the ground-truth instruction activates, and reconstruction error at the concept layer drops to baseline. On the steering-vector target, the contextualized soft prompt even surfaces persona-involvement features the steering vector itself does not, including a self-reference feature the model generates on its own. Contextualization makes soft prompts interpretable under both criteria. In the steering-vector case, it even reveals more structure than the target it was trained to match.

Three directions feel especially worth pursuing. First, contextualized soft prompts as an on-manifold alternative to activation steering, as proposed in the previous section. Training one on the same contrastive behavioral data that defines a persona vector would give a direct head-to-head comparison with the steering vector, and implicitly replicate the persona / assistant-axis results on an on-manifold surface. Second, contextualized soft prompts as a model diffing surface. Training one to match a behavioral gap is itself a form of model diffing at the prompt level: the soft prompt *is* the diff, expressed as an input-level vector rather than a weight change ([Diff Interpretation Tuning](https://openreview.net/forum?id=6As4wfTB77)) or activation pattern ([crosscoders](https://arxiv.org/abs/2504.02922)), and self-describing: the model can verbalize what changed. Third, compositionality. Do contextualized soft prompts compose? Can a second model interpolate between two of them and narrate the intermediate behaviors? What does the manifold of valid contextualized soft prompts look like in the first place?
